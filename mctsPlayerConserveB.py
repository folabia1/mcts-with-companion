...
def randomPolicy(node, player, currentDepth, targetDepth):
    ...
    if targetDepth:
        if currentDepth >= targetDepth or node.isTerminal:
            return currentDepth, node, node.state.calculateReward(player)[0]
        else:
            node.expand()
...
class MCTSPlayer():
    ...
    def executeRound(self, root, player, sightLimit):
        ...
        # simulation
        depth, simulationNode, reward = self.rollout(node, self.player, 0, sightLimit)
        # backpropagation
        self.backpropogate(simulationNode, reward, depth)
